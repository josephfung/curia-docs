...
Because the CLI is local-only, messages from it bypass the trust score thresholds that apply to external channels. You never need to worry about unknown-tier restrictions when using the CLI.

## When to use the CLI

- **Development and testing** — the fastest way to interact with Curia during development
- **Administrative tasks** — checking autonomy score, managing contacts, adjusting contact tiers
