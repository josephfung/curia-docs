...
## Step 2 — Seed the phone number into the vault

Set the number transiently and run the seed script. Use the same E.164 number you registered in Step 1:

```bash
SIGNAL_PHONE_NUMBER=+12223334444 pnpm run seed-vault
```

It is encrypted and stored under the vault key `signal_phone_number`.
...
<AccordionGroup>
  <Accordion title="Signal adapter not starting">
    ...
    1. `signal_phone_number` is in the vault (re-seed with `pnpm run seed-vault` if unsure)
