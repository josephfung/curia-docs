...
<Warning>
  `SIGNAL_SOCKET_PATH` is managed by Docker Compose and is set automatically when the `signal-cli` container starts. Do not set it in your `.env` file. The only credential you configure is the Signal phone number, which is stored in the [vault](/get-started/secrets-vault) as `signal_phone_number`.
</Warning>
...
In the Curia console, go to **Settings → Channels → Signal** and enter the E.164 number you registered in Step 1 (e.g. `+12223334444`). It is stored under the vault key `signal_phone_number`.
...
The Signal channel activates when `signal_phone_number` is in the vault and the `signal-data` volume is populated. `docker compose logs curia` will show the Signal adapter initializing.
...
<AccordionGroup>
  <Accordion title="Signal adapter not starting">
    ...
    1. `signal_phone_number` is in the vault (check Settings → Channels → Signal in the console)
