...
Recreate the Curia container so it picks up the new environment variable:

```bash
docker compose up -d --force-recreate curia
```

The Signal channel activates when `SIGNAL_PHONE_NUMBER` is set and the `signal-data` volume is populated. `docker compose logs curia` will show the Signal adapter initializing.
