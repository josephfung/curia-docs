...
<Steps>
  ...
  <Step title="Apply the change">
    Recreate the Curia container so it picks up the new environment variables:

    ```bash
    docker compose up -d --force-recreate curia
    ```

    The email channel activates automatically when all three Nylas variables are present. `docker compose logs curia` will show the email adapter initializing.
  </Step>
</Steps>
