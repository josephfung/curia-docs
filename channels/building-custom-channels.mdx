...
<Steps>
  <Step title="Implement the Channel interface">
    ...
    Register on the bus at the `channel` layer — the hard security boundary limits a channel to publishing `inbound.message` and subscribing to `outbound.message`. It cannot invoke tools, read memory, or trigger agent tasks.
  </Step>
