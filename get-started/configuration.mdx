...
<Note>
  First-time setup configures `.env` for you. `pnpm run setup` generates secrets and prompts for your Anthropic API key on the first install, and the in-app setup wizard captures your assistant's identity, voice, and decision posture conversationally. This page covers everything you might want to change later — additional channels, multiple email accounts, custom LLM routing, and manual `.env` edits.
</Note>

<Warning>
  Most configuration changes take effect on restart. After editing `.env` or any YAML file, recreate the Curia container to apply changes: `docker compose up -d --force-recreate curia`. The Tavily API key is the only exception — the `web-search` skill picks it up on next use without a restart.
</Warning>
...
<AccordionGroup>
  <Accordion title="Authentication and access">
    ...
    | `WEB_APP_BOOTSTRAP_SECRET` | Yes | Secret for logging in to the console at `/` (setup wizard, chat, tasks, knowledge graph, jobs, settings). Set to any long random string. |
