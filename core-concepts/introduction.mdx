...
Agents access credentials through a scoped interface that validates each request against the skill's declared manifest. The LLM sees "email-list connected to inbox" — never the actual password or API key. Every credential access is audit-logged.
