...
You can review all currently warned nodes at any time by asking Curia, which uses the `decay-warnings-list` tool. The `memory-confirm` tool records your decision and emits a `memory.decay_warning` audit event.
