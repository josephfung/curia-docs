...
Behind that, agents use the `task-create` / `task-list` / `task-update` / `task-complete` skills. A task can carry a **wake time** — when it's time to act, the scheduler fires and the owning agent resumes with the task's full context. You can review the backlog any time — what's for you to do, what's waiting on others, and what Curia is working on.
