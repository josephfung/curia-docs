...
The upshot for you is visible progress. When you ask "what's open?" or read the daily digest, a project reports as **"3 of 5 steps done"** rather than one opaque line, so you can see how far along it is and what it's waiting on.
...
Long-running work doesn't always go smoothly, and Curia is explicit when something needs you. Rather than a bare "still working" line, a stuck task escalates into the daily digest with the *reason* and enough detail to act on. There are four shapes:
...
Each escalation carries where it got to (progress so far, and a rough ETA or cost where relevant) plus a suggested next action — so the digest tells you what to do about it, not just that it happened.
