...
Cron expressions run in Curia's configured timezone (set via `TIMEZONE` in `.env`). If you need a specific job to run in a different timezone, the scheduler-create tool accepts an optional `timezone` parameter.
...
Agents can manage jobs programmatically using these built-in tools:
...
The `scheduler-create` tool accepts these inputs:
...
### Projects: breaking a goal into steps

Not every task is a single stretch of work. When a goal is genuinely multi-step — plan an offsite, run a diligence process, prepare a board packet — an agent can break it into a small set of child steps with dependencies, then work them in order as earlier steps unblock later ones. You don't set this up; the agent decides when a goal is big enough to warrant it.

The upshot for you is visible progress. When you ask "what's open?", a project reports as **"3 of 5 steps done"** rather than one opaque line, so you can see how far along it is and what it's waiting on.
...
- **You** — things only you can do (ask "what's waiting on me?" to see them).
...
Behind that, agents use the `task-create` / `task-list` / `task-update` / `task-complete` tools. A task can carry a **wake time** — when it's time to act, the scheduler fires and the owning agent resumes with the task's full context. You can review the backlog any time — what's for you to do, what's waiting on others, and what Curia is working on.
...
A deterministic **hourly heartbeat** is the backstop that keeps open work from lingering. It does no AI reasoning — it simply notices tasks that have gone quiet (an idle task Curia owns, or a "waiting" task whose expected reply never came) and wakes the owning agent to advance or re-evaluate them. Most work actually advances faster than that, driven by incoming replies and per-task wake timers; the heartbeat only catches the stragglers. Task management is enabled per agent by pinning the `tasks` skill (`pinned_skills: [tasks]`), which also makes the agent heartbeat-eligible, and tuned under the `tasks` block in `config/default.yaml`.

### When a task gets stuck

Long-running work doesn't always go smoothly, and Curia is explicit when something needs you. Rather than a bare "still working" line, a stuck task escalates to you with the *reason* and enough detail to act on. There are four shapes:

- **Stalled** — it stopped making progress, so Curia paused it instead of spinning.
- **Hit a limit** — it reached a configured ceiling (time, cost, or number of attempts).
- **Blocked on a person** — it's waiting on a reply or a decision from someone.
- **Couldn't finish** — the agent tried and reported it can't complete the work as scoped.

Each escalation carries where it got to (progress so far, and a rough ETA or cost where relevant) plus a suggested next action — so the escalation tells you what to do about it, not just that it happened.
...
The fence is a prompt-level guard, not a code-level one. It catches the dominant drift pattern without restricting legitimate work — but for security-critical operations (outbound sends, calendar writes), defense-in-depth at the tool layer remains the right counter to adversarial or buggy behavior.
...
The web console has a **Jobs** view that lists every scheduled job. You can filter by schedule type and by agent, read a plain-language summary of each schedule, and jump straight from a job to the tasks it has spawned via linked task titles.
...
Jobs that fail 3 consecutive times are automatically **suspended**. When this happens, Curia sends you a direct email notification — it bypasses the LLM pipeline entirely so the alert is delivered even if the Anthropic API is unavailable (the most common cause of repeated job failures). The email identifies the job, the agent, the failure count, and the last error, and includes the job's objective and recurrence plus a deep link that opens that job directly in the console's Jobs view.
