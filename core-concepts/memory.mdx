...
Open [http://localhost:3000](http://localhost:3000) and log in with your `WEB_APP_BOOTSTRAP_SECRET` (generated by `pnpm run setup` and stored in `.env`). From the sidebar, choose **Knowledge graph** — or navigate to `/kg` directly.

<Info>
  If `WEB_APP_BOOTSTRAP_SECRET` is not set in your environment, the knowledge-graph API is intentionally disabled. Set the variable and recreate the Curia container to enable it.
</Info>
