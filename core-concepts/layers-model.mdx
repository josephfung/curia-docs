...
description: "How Curia's governance layers compose — from channel trust to dispatch policy, coordinator autonomy checks, and skill-level action risk."
---
...
Messages below the trust score floor (default: 0.2) are held regardless of channel policy.
...
The Coordinator agent receives the task with the current [autonomy score](/core-concepts/autonomy) injected into its system prompt. The autonomy band determines how independently the agent can act:

- At **Full** (90–100): proceeds on standard operations without confirmation
- At **Approval Required** (70–79): presents a plan and asks before any consequential action
- At **Restricted** (< 60): advisory only, takes no independent action
