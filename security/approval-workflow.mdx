...
An approval request is created any time the [execution layer](/security/autonomy-engine) blocks a tool invocation because the live score is below the tool's `action_risk` floor. Common situations:
...
- **Score is in Approval Required (70–79):** A tool with `action_risk: high` (e.g., creating a calendar event on your behalf) is invoked. The gate allows medium-risk tools but blocks high-risk ones — an approval request is raised.
- **Any score, blocked tool:** A tool with `action_risk: critical` is invoked at any score below 90. The request captures the full context so you can evaluate it.
...
- The tool name and what it was trying to do
...
You can also use the `list-pending-actions` tool directly.
</Tip>
...
2. Re-executes the tool with `humanApproved: true`, which bypasses the gate for this invocation only
...
- Standard expirations close quietly and are recorded in the action log

**Staying aware:** Ask Curia **"what's waiting for my approval?"** any time to see everything still open, or fold outstanding approvals into an [on-demand morning briefing](/examples/daily-morning-briefing) so nothing lingers unseen.
...
<CardGroup cols={2}>
  ...
  <Card title="Built-in tools" icon="puzzle-piece" href="/tools-and-skills/built-in-tools">
    The approve-action, deny-action, dismiss-action, and list-pending-actions tools.
  </Card>
</CardGroup>
