...
| `secret.accessed` | Every `ctx.secret()` call — the secret name (never the value), the requesting skill and task, and whether the value resolved from the vault or an env fallback (`source: vault \| env`) |
...
When a skill calls `ctx.secret()` to access a credential from the [vault](/get-started/secrets-vault), the access is audit-logged as a `secret.accessed` event before the secret value is returned. The log entry records:
...
Secret values are never written to the audit log. The log tells you that the email-get skill accessed the Nylas credential — not what that credential is.
...
- **Track secret accesses** — filter for `secret.accessed` events to see which credentials were used, by which skill, in which task
