...
<CardGroup cols={2}>
  ...
  <Card title="Container and supply chain" icon="container-storage">
    The production Docker image runs as a non-root `curia` user. Automated security scanning runs on every pull request: Trivy (npm deps, Docker image, secrets), Semgrep (pattern-based SAST), CodeQL (semantic analysis), and Gitleaks (secret detection). Branch protection on `main` requires PR review and passing status checks before merge.
  </Card>
</CardGroup>
