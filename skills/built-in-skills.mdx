...
| Skill | action_risk | Description |
|---|---|---|
| `email-send` | medium | Send a new email to specified recipients. Accepts an optional `context_bridge` JSON for delegation-aware reply routing. |
| `email-reply` | medium | Reply to an existing email thread. Appends the quoted original message body to the reply (matching standard email-client behaviour). Accepts an optional `context_bridge` JSON. |
