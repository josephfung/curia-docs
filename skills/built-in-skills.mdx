...
| `contact-set-tier` | low | Set a contact's capability tier (e.g., known, trusted) |
...
| `approve-grant-recommendation` | low | Approve a pending grant recommendation and immediately grant the underlying permission to the contact |
| `decline-grant-recommendation` | low | Decline a pending grant recommendation; permanently suppresses it so Curia never surfaces that recommendation again |
...
| `web-browser` | medium | Interact with JavaScript-rendered pages via Playwright |
