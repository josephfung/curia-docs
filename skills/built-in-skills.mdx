...
| `context-bridge-clear` | none | Release all active outbound-context entries for a named meeting subject across the full active table; used by the meeting-debrief agent after "clear meeting X" to ensure every entry is freed, not just the one in the injected window |
