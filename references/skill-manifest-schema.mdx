...
`elevated` is the live-principal gate: the skill runs only when the current turn originated from a fresh CEO inbound (or a synchronous delegation within one). It is not a one-time "approve once" prompt. See [Sensitivity](/skills/how-skills-work#sensitivity-normal-and-elevated) for the full model.
...
This is the *runtime* allowlist — what the handler may read while executing. To require that a secret be **present before the skill can be installed or enabled**, declare it in [`install.requires_secrets`](#install) instead (a skill commonly declares the same key in both).
...
| `taskRepo` | Task backlog read/write — used by the `task-*` skills and the dedup scanner |
| `secretCapture` | Mint-only minter for agent-initiated secret-capture links; returns a one-time link, never the secret value. Restricted to the capture-request skills. |
...
Optional. Declares install-time requirements that the registry enforces before a skill can be installed or enabled. Skills live in a [DB-backed registry](/core-concepts/registry) and only enabled skills are loaded at startup.

| Field | Description |
|---|---|
| `requires_secrets` | Array of vault secret keys that must already exist in the [vault](/get-started/secrets-vault) before the skill can be installed or enabled. The registry rejects the install/enable until every listed key is present. |
