...
Controls the headless browser the `web-browser` skill drives (Playwright). Sessions are pooled and swept on a timer so logins can persist across calls without leaking idle browser processes.
...
browser:
  ...
  channel: ""              # "chrome" for real Chrome (must be in the image); empty → bundled Playwright Chromium
