...
sidebarTitle: "GET /api/messages/stream"
...
| `tool.invoke` | The agent is calling a tool (e.g. fetching calendar, sending email) |
| `tool.result` | A tool call has completed and returned a result |

Watching `tool.invoke` and `tool.result` events lets you show the user what the agent is doing while it works — useful for tasks that chain multiple tools together.
...
```
data: {"type":"tool.invoke","tool":"get-calendar-events","conversationId":"my-conv-id"}

data: {"type":"tool.result","tool":"get-calendar-events","conversationId":"my-conv-id","success":true}
