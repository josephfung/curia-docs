...
| `threaded` | `true`, `false` | `true` when the platform has native threads (email, Slack). `false` enables [context bridging](/channels/how-channels-work#context-bridging-on-non-threaded-channels) for channels without a thread reference (Signal, CLI, HTTP). |

Current defaults: CLI and Signal are `high`, HTTP and Slack are `medium`, email is `low`. HTTP uses `unknown_sender: ignore`; the rest `allow`.
